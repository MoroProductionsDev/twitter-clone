...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    ...
    func postTweet(tweetString : String, success: @escaping() -> (), failure: @escaping(Error) -> ()) {
        let UPDATE_URL = "https://api.twitter.com/1.1/statuses/update.json"
        
        TwitterAPICaller.client?.post(UPDATE_URL, parameters: ["status":tweetString], progress: nil,
    ...
    func favoriteTweet(tweetId : Int, success: @escaping () -> (), failure: @escaping (Error) -> ()) {
        // API URL for creation
        let RESOURCE_URL_FAV_CREATE = "https://api.twitter.com/1.1/favorites/create.json"
        
        TwitterAPICaller.client?.post(RESOURCE_URL_FAV_CREATE, parameters: ["id" : tweetId], progress: nil,
    ...
    func unfavoriteTweet(tweetId : Int, success: @escaping () -> (), failure: @escaping (Error) -> ()) {
        // API URL for destruction
        let RESOURCE_URL_FAV_DESTROY = "https://api.twitter.com/1.1/favorites/destroy.json"
        
        TwitterAPICaller.client?.post(RESOURCE_URL_FAV_DESTROY, parameters: ["id" : tweetId], progress: nil,
                                      success: { (task: URLSessionDataTask, response: Any? ) in
                                        success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
    ...
    func retweet(tweetId: Int, success: @escaping () -> (), failure: @escaping (Error) -> ()) {
        // API URL for destruction
        // :id means that it has to be replaced with a variable
        let RESOURCE_URL_RETWEET = "https://api.twitter.com/1.1/statuses/retweet/\(tweetId).json"
        
        TwitterAPICaller.client?.post(RESOURCE_URL_RETWEET, parameters: ["id" : tweetId], progress: nil,
                                      success: { (task: URLSessionDataTask, response: Any? ) in
                                        success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
